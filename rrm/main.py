...
def main(env, replay_buffer, is_atari, state_dim, num_actions, args, parameters, device):
	...
	np.savez(
		f"./results/{setting}/episodic_returns.npz",
...
if __name__ == "__main__":
	...
	parser.add_argument("--rrm", default=False, action="store_true")
	parser.add_argument("--stratified", default=False, action="store_true")
	args = parser.parse_args()

	args.algorithm = "LAP_DDQN"

	print("---------------------------------------")	
	print(f"Setting: Algorithm: {args.algorithm}, RR-M: {args.rrm}, Stratified: {args.stratified}, Env: {args.env}, Seed: {args.seed}")
	print("---------------------------------------")

	setting = f"{args.algorithm}_{args.rrm}_{args.stratified}_{args.env}_{args.seed}"

	if not os.path.exists(f"./results/{setting}"):
		os.makedirs(f"./results/{setting}")

	metadata = {**vars(args)}
	with open(f"./results/{setting}/metadata.json", "w") as f:
